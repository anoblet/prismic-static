...
let { reference, repository, type } = arguments;
...
(async () => {
  ...
  const { masterRef, types } = await getStructure();
  reference = reference || masterRef;

  if (type) {
  } else {
    types.map(async (type) => {
      fs.mkdir(`prismic_static/${type}`, () => null);
      const documents = await getDocumentsByType({ reference, type });
